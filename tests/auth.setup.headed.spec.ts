...
test.describe('auth setup (headed/debug preserved)', () => {
	test('login and extract via headed flow', async ({ page, context }) => {
		...
		const googleBtn = googleSsoButton.first();
		const msBtn = microsoftSsoButton.first();
		...
		if (wantGoogle && googleReady) {
			...
			console.info('[auth] Attempting Microsoft SSO login...');
			...
			console.info('[auth] Attempting password-based login...');
